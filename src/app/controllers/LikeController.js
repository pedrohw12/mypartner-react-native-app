...
class LikeController {
  async store(req, res) {
    const { user_liked_id } = req.params; // id do usuário que quero dar like
    const userLogged = await User.findByPk(req.userId); // o id de quem irá fazer a ação de dar like
    const userLiked = await User.findByPk(user_liked_id); // achando o usuário que recebeu like
    ...
    await Like.create({ userLogged, user_liked_id });
